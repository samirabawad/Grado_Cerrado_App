...
export class CivilReforzarPage implements OnInit {
  ...
  async loadTemasFromDatabase() {
    try {
      ...
      console.error('âŒ Error cargando temas de Civil desde BD:', error);
      this.temas = [];
    }
  }
  ...
  async startTest() {
    ...
    try {
      ...
      if (sessionResponse?.success) {
        ...
        await loading.dismiss();
        console.error('âŒ Error en respuesta:', sessionResponse);
      ...
      await loading.dismiss();
      console.error('âŒ Error al iniciar test:', error);
